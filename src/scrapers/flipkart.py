...
Usage:
    python -m gem_price.scrapers.flipkart --query "office chair" --max-results 30
...
# More robust price regex - handles ₹, commas, decimals
...
def load_search_page(driver, query: str, page: int = 1) -> Optional[str]:
    from selenium.webdriver.common.by import By
    ...
    try:
        ...
        logger.error(f"navigation error on '{query}' p{page}: {type(e).__name__}: {e}")
        return None
    time.sleep(random.uniform(settings.flipkart_page_delay_min, settings.flipkart_page_delay_max))
    if detect_block(driver):
        return None
    # lazy-loaded results: scroll to trigger more cards, capped
    for _ in range(settings.flipkart_scroll_count):
        try:
            ...
            logger.warning(f"scroll error: {type(e).__name__}: {e}")
            break
...
def parse_cards_from_html(html: str) -> List[dict]:
    ...
    cards = soup.select("div.nZIRY7")  # Current primary
    if not cards:
        cards = soup.select("div[data-id]")  # Fallback 1
    if not cards:
        cards = soup.select('a[href*="/p/"]')  # Fallback 2
    if not cards:
        cards = soup.select("div._1AtVbE")  # Older layout
    
    out = []
    for card in cards:
        # Try multiple name selectors
        name_el = card.select_one("div.RG5Slk, div._4rR01T, div.KzDlHZ, div.syl9yP, div._2B099V, a[title]")
        if name_el:
        ...
        price = None
        price_el = card.select_one("div.hZ3P6w.DeU9vF, div.hZ3P6w, div.Nx9bqj, div._30jeq3, div._1_WHN1")
        if price_el:
...
def scrape_flipkart(query: str, max_results: int = 30,
                    searched_for_gem_id: Optional[int] = None) -> List[dict]:
    from selenium import webdriver
    ...
    try:
        for page in range(1, 4):  # up to 3 pages to guarantee >=15 on lazy layout
            ...
            if html is None:
                logger.warning("BLOCKED/CAPTCHA on search page — stopping per guardrail, no bypass attempted.")
                debug = RAW_DIR / "flipkart_debug_block.html"
                debug.write_text(driver.page_source, encoding="utf-8")
                logger.info(f"saved block sample to {debug}")
                break
            cards = parse_cards_from_html(html)
            if not cards:
                if page == 1:
                    logger.warning("no cards parsed — selectors may have changed; saving debug sample.")
                    debug = RAW_DIR / f"flipkart_debug_{re.sub(r'[^a-z0-9]+', '_', query.lower()).strip('_')}.html"
                    debug.write_text(html, encoding="utf-8")
                    logger.info(f"saved debug HTML to {debug}")
                else:
                    logger.info(f"page {page}: no cards, stopping.")
                break
            before = len(results)
            for c in cards:
                key = (c["name"], c["price"])
                ...
                if len(results) >= max_results:
                    break
            logger.info(f"page {page}: parsed {len(cards)} cards, +{len(results) - before} new (total {len(results)})")
            time.sleep(random.uniform(settings.flipkart_page_delay_min, settings.flipkart_page_delay_max))
    finally:
        driver.quit()
    return results
