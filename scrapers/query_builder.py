"""Query generation for per-product Flipkart lookup (architecture fix).
...
TAIL_NOISE = {
    ...
    "tray", "rack", "stand", "foam", "fabric", "gb", "tb",
}

BRAND_HINTS = {"hp", "dell", "lenovo", "acer", "asus", "apple", "samsung",
...
def _brand_from(segments: list[str], name: str) -> str:
    """First known-brand token (case-insensitive) in slug or display name."""
    for tok in segments:
        if tok.lower() in BRAND_HINTS:
            return tok.lower()
    low = (name or "").lower()
...
def build_queries(name: str, link: str) -> list[str]:
